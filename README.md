...
- **Discord slash commands**, gated by Discord role:
  - **Admin role**: `/join`, `/restart`, `/map <name|workshop-id>`, `/gamemode <mode> [map]`, `/update`, `/reinstall-plugins`, `/status`
...
- **Won't interrupt an active game.** Before the daily update, a manual `/update`, a manual `/reinstall-plugins`, or an hourly recovery restart actually stops/restarts the server, the bot checks the current player count over RCON (`status`). If anyone's connected, it defers — rechecking every `update.player_check_interval_seconds` (default 60) — and only proceeds once the server is empty, posting a status-channel notice when it starts deferring and when it resumes. In every case the server is then fully stopped before steamcmd or a plugin extraction touches the install, so no game files are in use while they're rewritten. Only `/restart` skips the empty-server check — it's the deliberate "restart right now" command, and doesn't rewrite any files.
...
hourly       not broken → no-op
             broken → any plugin released a newer version than installed?
                       ...
                       └─ yes → players online? defer, notify ⏸️/▶️ → stop → install it → restart WITH plugins → verify
